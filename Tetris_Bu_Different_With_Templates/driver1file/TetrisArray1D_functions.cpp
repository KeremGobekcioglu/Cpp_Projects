...
void TetrisArray1D::move(Tetromino &teto)
{
	...
	// numberofmoves++;
	// setstring(whichway,howmanysquare);
...
// {
// 	cout<<"width = "<<board_width<<endl<<"suradayim widthe bol a bol mod al cart curt = "<<(rowposition + teto.getwhich().size())*this->width() + columnposition<<endl;
...
// 			return false;
// 	return true;
bool TetrisArray1D::controlspace(Tetromino &teto, const int rowposition, const int columnposition) const
{
	// if (rowposition + 1 >= this->board_size / this-> board_width)
		// return false;
	// if shape can add , it will return true
	for (int j = 0; j < teto.getwhich()[0].size(); j++)
		if ((board[((rowposition + 1 + teto.getwhich().size())*this->width()) + columnposition + j] != '.' ))//&& teto.getwhich()[teto.getwhich().size() - 1][j] != '.'))
			return false;
	return true;
	// int c = 0;
	// // cout<<"columnposition = "<<columnposition<<endl<<"rowposisition + tetosize is = "<<rowposition+ teto.getwhich().size()<<endl;
	// for (int i = 0; i < teto.getwhich()[0].size(); i++)
	// 	if (teto.getwhich()[teto.getwhich().size() - 1][i] == '.' && board[rowposition + 1][columnposition + i] == '.')
	// 		c++;

	// if(c == teto.getwhich()[0].size()) return false;
	// else return true;
}
...
// satır 5 column 9 olsun 3. satırın 4. elemanı 
